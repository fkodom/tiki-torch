...
def detections_dict_to_array(detections: Dict) -> np.ndarray:
    r"""Accepts a dictionary of detection values (like the one returned by
    utils.detect.get_detections), and returns an array of detection locations.

    :param detections: Dictionary of detection centroid locations
    :return: Array of detection centroid locations. Shape: (N, 3). Columns contain:
        (0) frame
        (1) row
        (2) column
    """
    ...
    for f in detections.keys():
        ...
        detections_array.append(
            np.stack(
                (
                    frames,
                    np.array(detections[f]["rows"]),
                    np.array(detections[f]["cols"]),
                ),
                1,
            )
        )
...
def detections_array_to_dict(detections: np.ndarray) -> Dict:
    r"""Accepts an array of detection values, and returns a dictionary of
    detection locations (like the one returned by utils.detect.get_detections).
...
def target_distances(
    detect_positions: np.ndarray, target_positions: np.ndarray
) -> np.ndarray:
    r"""Computes the distance between every possible detection-target pair,
    across all frames.  Detection-target pairs with different frame numbers are
    considered to be an *infinite* distance apart (requires detection to occur
    in the correct frame to count as a true detection). Distance is then computed
    ...
    :return: Distances between every detection-target pair.
        Shape: (num_detect_points, num_target_points)
    """
    displacements = target_positions[:, 1:].reshape(-1, 1, 2) - detect_positions[
        :, 1:
    ].reshape(1, -1, 2)
    frame_differences = target_positions[:, 0].reshape(-1, 1) - detect_positions[
        :, 0
    ].reshape(1, -1)
...
def detection_inside_datacube(
    detect_positions: np.ndarray, datacube_size: Tuple[int, int, int]
) -> np.ndarray:
    r"""Checks to see if each point is inside of the datacube.  (It is possible
    to get detections just outside of the datacube when using
    skimage.measure.regionprops for detections, which we do.)

    :param detect_positions: Array of detection centroid locations.  Shape: (N, 3)
    :param datacube_size: Size of the original datacube.  Shape: (nframe, nrow, ncol)
    :return: Boolean array, describing whether each detection is inside the
        datacube.  Shape: (N, )
    """
...
def detection_snr(
    ...
    r"""Computes the summed-box SNR for each detection, using pixel values from
    a PCBS datacube.  Signal is computed as the sum of an interior circle,
    which extends out to 'inner_radius'.  Noise is computed as the standard
    deviation a ring around the interior circle, which starts at distance
    'inner_radius', and extends out to 'outer_radius'.
    ...
    chips = np.stack(
        tuple(
            x[frame, row : row + w, col : col + w]
            for frame, row, col in reduced_positions
        ),
        0,
    )
...
def is_target(
    ...
    r"""For each detection, computes the summed-box SNR and the distance to all
    known target positions.  Determines whether each detection corresponds to a
    real target, using user-specified minimum values for SNR, distance.  Only
    associates a maximum of 'max_detects' detections with each known target, to
    avoid over-counting.
    ...
    :param pcbs_datacube: Datacube after performing running PCBS for background
        suppression. Shape: (nframe, nrow, ncol)
    :param max_distance: Maximum distance from a known target, in polynomial_order
        to be considered a true detection.
    ...
    :return: Boolean array, describing whether each detection corresponds to a
        known target.  Shape: (N, )
    """
...
def get_true_positives(
    ...
    r"""Counts the number of 'True Positive' detections contained in the
    'detect_positions' array.  A true positive is a detection, which can be
    associated with a nearby, known target position.
...
def get_false_positives(
    ...
    r"""Counts the number of 'False Positive' detections contained in the
    'detect_positions' array.  A false positive is a detection, which can NOT
    be associated with a nearby, known target position.
...
def get_truth_datacube(detections: Dict, size):
    r"""Given a dictionary of detection centroid locations, reconstructs a
    binary truth datacube.
